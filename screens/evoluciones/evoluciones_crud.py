...
from services.signo_vital_service import get_signos_vitales_by_paciente
from services.diagnostico_service import get_diagnosticos_by_consulta
from services.prescripcion_service import get_prescripciones_by_consulta
from services.tratamiento_service import get_tratamientos_by_consulta
...
def obtener_historias_clinicas(id_usuario, search_query=""):
    ...
    for historia in historias:
        antecedentes = get_antecedentes_medicos_by_paciente(historia.id_paciente, id_usuario)
        signos_vitales = get_signos_vitales_by_paciente(historia.id_paciente, id_usuario)
    ...
    if search_query:
        ...
        historias_filtradas = [
            h for h in historias_filtradas
            if (normalized_query in normalize_string(get_paciente(h.id_paciente).nombre)) or
               (normalized_query in normalize_string(get_paciente(h.id_paciente).apellido))
        ]
