...
def crear_formulario_tamizaje(page, add_tamizaje_clicked, on_paciente_search, select_paciente, all_tamizajes):
    # Campos del formulario
    tamizaje_paciente = ft.TextField(label="ID Paciente", width=200, read_only=True)
    tamizaje_tipo = ft.TextField(label="Tipo de antecedente médico", expand=True)
    tamizaje_descripcion = ft.TextField(label="Descripción del antecedente médico", expand=True)
    tamizaje_fecha = ft.TextField(label="Fecha del signo vital", expand=True)
    tamizaje_presion_arterial = ft.TextField(label="Presión arterial", expand=True)
    tamizaje_frecuencia_cardiaca = ft.TextField(label="Frecuencia cardíaca", expand=True)
    tamizaje_frecuencia_respiratoria = ft.TextField(label="Frecuencia respiratoria", expand=True)
    ...
    form_content = ft.Column(
        [
            ...
            paciente_results,
            ft.Row([tamizaje_paciente, tamizaje_tipo, tamizaje_descripcion], spacing=15),
            ft.Row([tamizaje_fecha, tamizaje_presion_arterial, tamizaje_frecuencia_cardiaca], spacing=15),
            ft.Row([tamizaje_frecuencia_respiratoria, tamizaje_temperatura, tamizaje_peso, tamizaje_talla], spacing=15),
